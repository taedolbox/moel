...
def realjob_application_app():
    ...
    # 사이드바 (이미지 기반으로 메뉴 및 입력 필드 구성)
    with st.sidebar:
        st.markdown("### 실업인정")
        st.text_input("구직번호", value="G10000H1EE")
        st.text_input("구직일자", value="YYYY-MM-DD")
        st.selectbox("사유확인", ["사유확인", "사유1", "사유2"])
        st.markdown("### 구직활동")
        st.text_input("PRBCON-SK 08")
        st.text_input("~SOCK-SK 01")
        st.markdown("### 연락처")
        st.text_input("연락처", value="010-1234-5678")
        st.markdown("### 기타")
        st.text_input("입력값", value="입력값 예시")
    ...
    st.markdown("### 구직활동 정보 *")
    with st.container():
        col1, col2 = st.columns([1, 1])  # 열 너비 균등 조정
        with col1:
